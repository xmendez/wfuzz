                ...
                with mock.patch('os.path.isfile') as mocked_isfile:
                    with mock.patch('imp.find_module') as mocked_find_module:
                        with mock.patch('imp.load_module') as mocked_load_module:
                            ...
                            mocked_find_module.return_value = (None, '/any/alpha/project.py', ('.py', 'U', 1))
                            mocked_load_module.return_value = sys.modules[__name__]

                            br = BRegistrant(DirLoader(**{"base_dir": 'beta', "base_path": 'any'}))

                            self.assertEqual(sorted(br.get_plugins_names()), sorted(['test_plugin1', 'test_plugin2', 'test_plugin3']))
                            self.assertEqual(br.get_plugins_names('default'), ['test_plugin2', 'test_plugin3'])
                            self.assertEqual(br.get_plugins_names('aggressive'), ['test_plugin1'])
                            self.assertEqual(sorted(br.get_plugins_names('not aggressive')), sorted(['test_plugin2', 'test_plugin3']))
                            self.assertEqual(sorted(br.get_plugins_names('default or aggressive')), sorted(['test_plugin1', 'test_plugin2', 'test_plugin3']))
                            self.assertEqual(sorted(br.get_plugins_names('default and safe')), sorted(['test_plugin3']))
                            self.assertEqual(sorted(br.get_plugins_names('test_pl*')), sorted(['test_plugin1', 'test_plugin2', 'test_plugin3']))
                            self.assertEqual(sorted(br.get_plugins_names('test_plugin1')), sorted(['test_plugin1']))
