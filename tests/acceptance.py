...
class APITests(unittest.TestCase):
    def test_get_payload(self):
        payload_list = wfuzz.get_payload(range(4)).data.get('dictio')[0]
        self.assertEqual(sorted(payload_list), sorted([0, 1, 2, 3]))

    def test_get_payloads(self):
        payload_list = wfuzz.get_payload([range(2), range(2)]).data.get('dictio')[0]
    ...
    def test_encoders(self):
        ...
        for key, values in [items for items in encoders.items() if items[0] not in ['html_hexadecimal', 'html_decimal', 'md5', 'sha1', 'html_escape']]:
            value, expected = values
