...
class headers(BasePlugin):
    ...
    version = "0.1"
    summary = "Looks for new response headers associated to HTTP servers in HTTP responses"
        ...
        for header in self.watch_headers:
            ...
            if header_value is not None:
                if header_value.lower() not in self.kbase[KBASE_KEY] or KBASE_KEY not in self.kbase:
                    self.add_result("New server response header. {}: {}".format(header, header_value))
