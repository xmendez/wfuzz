...
class backups(BasePlugin):
	...
	return fuzzresult.code != 404 and (fuzzresult.history.urlparse.file_extension not in self.extensions)
	...
	for pre, extension in self.extensions:
	    ...
	    self.queue_url(urljoin(fuzzresult.url, pre + fuzzresult.history.urlparse.file_name + extension))

	    # http://localhost/dir/test.html ---> test.html.BAKKK
	    self.queue_url(urljoin(fuzzresult.url, fuzzresult.history.urlparse.file_fullname + extension))
